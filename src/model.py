...
def mlp(x, scope, n_state, *, hparams):
    with tf.variable_scope(scope):
        ...
        if hparams.tpu_address is not None and hparams.shards > 0:
            ...
            h2 = tf.contrib.tpu.shard(op, [fc_w, fc_b, pr_w], input_shard_axes=[-1, -1], output_shard_axes=[2], num_shards=hparams.shards, device_assignment=get_tpus(hparams))
            if 'GPT2_DEBUG' in os.environ:
...
def model(hparams, X, past=None, scope='model', reuse=tf.AUTO_REUSE, align=False):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        if hparams.tpu_address is not None and hparams.shards > 0 and align:
            ...
            output_reduce_axis = -1 if not 'GPT2_OUTPUT_REDUCE_AXIS' in os.environ else int(os.environ['GPT2_OUTPUT_REDUCE_AXIS'])
            logits0 = tf.contrib.tpu.shard(op, [wte], input_shard_axes=[input_shard_axis_0], output_shard_axes=[output_shard_axis_0], num_shards=hparams.shards, device_assignment=get_tpus(hparams))
            if output_reduce_axis >= 0:
